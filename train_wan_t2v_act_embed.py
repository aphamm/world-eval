...
class TextVideoDataset(torch.utils.data.Dataset):
    ...
    def load_hdf5(self, file_path):
        with h5py.File(file_path, "r") as f:
            ...
            start_frame_id = torch.randint(
                0, total_frames - (self.num_frames - 1) * self.frame_interval, (1,)
            ).item()
            end_frame_id = start_frame_id + self.num_frames * self.frame_interval

            frames = self.load_frames_from_hdf5(
                data, start_frame_id, end_frame_id, is_compressed=is_compressed
            )
    ...
    def load_act_embed(self, file_path, start_index, end_index):
        if self.action_encoded_path is not None:
            ...
            with h5py.File(file_path, "r") as f:
                action_data = f["action_embed"][
                    start_index : end_index + 1 : self.frame_interval
                ]
    ...
    def load_text_from_hdf5(self, file_path, start_frame_id, end_frame_id):
        with h5py.File(file_path, "r") as f:
            ...
            if (
                ...
                texts = [
                    f["substep_reasonings"][i].decode("utf-8").strip()
                    for i in range(start_frame_id, end_frame_id, self.frame_interval)
                ]
    ...
    def load_frames_from_hdf5(
        self, data, start_frame_id, end_frame_id, is_compressed=False, image_size=None
    ):
        ...
        if is_compressed:
            frames = [
                cv2.imdecode(np.frombuffer(frame.tobytes(), np.uint8), cv2.IMREAD_COLOR)
                for frame in frames
            ]
    ...
    def __getitem__(self, data_id):
        ...
        if self.is_image(path):
            ...
            to_path = os.path.join(
                self.base_path,
                "train",
                f"{parent_dir_name}_{self.file_name[actual_data_id]}_{same_file_sample_id}",
            )
        else:
            # mp4
...
class TensorDataset(torch.utils.data.Dataset):
    def __init__(self, base_path, metadata_path, steps_per_epoch):
        ...
        self.path = [
            os.path.join(base_path, "train", file_name)
            for file_name in os.listdir(os.path.join(base_path, "train"))
            if file_name.endswith(".tensors.pth")
        ]
    ...
    def add_lora_to_model(
        ...
        if pretrained_lora_path is not None:
            ...
            state_dict = {
                ...
                and not k.startswith("action_proj")
                or k in model.state_dict()
            }
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path of latent action data exported from the policy encoder.",
    )
    parser.add_argument(
        "--encode_mode",
        type=str,
