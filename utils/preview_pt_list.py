...
def preview_pt_file(pt_path, max_items=10):
    ...
    try:
        ...
        if not isinstance(data, (list, dict)):
            print(
                f"Warning: Loaded data is not a list or dictionary type, but {type(data)}"
            )
            return
        ...
        if isinstance(data, list):
            for i, item in enumerate(data[:max_items]):
                ...
                if isinstance(item, (torch.Tensor, list, tuple, dict)):
                    print(
                        f"Shape/Length: {len(item) if isinstance(item, (list, tuple, dict)) else item.shape}"
                    )
            ...
            for i, (key, value) in enumerate(data.items()):
                ...
                if isinstance(value, (torch.Tensor, list, tuple, dict)):
                    print(
                        f"Shape/Length: {len(value) if isinstance(value, (list, tuple, dict)) else value.shape}"
                    )
                ...
                if isinstance(value, list) and len(value) > 0:
                    ...
                    print(
                        f"First item shape: {value[0].shape if isinstance(value[0], (torch.Tensor, numpy.ndarray)) else 'N/A'}"
                    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Maximum number of list items to display",
    )
