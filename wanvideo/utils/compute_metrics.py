...
def read_frames_from_hdf5(hdf5_file_path, num_frames=81, start_frame=0):
    ...
    with h5py.File(hdf5_file_path, "r") as hdf5_file:
        ...
        frames = [
            np.array(images[i]) for i in range(start_frame, start_frame + num_frames)
        ]
    return frames
...
def main():
    ...
    for item in metadata:
        ...
        video_file_path = os.path.join(
            directory,
            subdirectory,
            filename.replace(".png", "_video.mp4")
            if subdirectory != "v1"
            else filename.replace("_v1.png", "_video.mp4"),
        )
        ...
        hdf5_frames = read_frames_from_hdf5(
            hdf5_file_path, num_frames=81, start_frame=start_frame
        )
    ...
    if len(real_images) > 0 and len(gen_images) > 0:
        ...
        for i in range(0, len(real_images), batch_size):
            ...
            batch_tensor = torch.stack(
                [torch.from_numpy(img).permute(2, 0, 1) for img in batch]
            ).to(device)
